...
function StatusBadge({ status }: { status: ReadingItem['status'] }) {
  if (status === 'reading') {
    return (
      <span className="inline-flex items-center gap-1.5 text-[11px] font-medium px-2 py-0.5 rounded-full
        bg-fuchsia-400/15 text-fuchsia-500 dark:text-fuchsia-400 border border-fuchsia-400/25"
      >
  ...
  if (status === 'discussing') {
    return (
      <span className="inline-flex items-center gap-1.5 text-[11px] font-medium px-2 py-0.5 rounded-full
        bg-amber-400/15 text-amber-600 dark:text-amber-400 border border-amber-400/25"
      >
        <span className="relative flex h-1.5 w-1.5">
  ...
  return (
    <span className="text-[11px] font-medium px-2 py-0.5 rounded-full
      bg-white/20 dark:bg-white/[0.06] text-gray-600 dark:text-white/65
...
function TypeBadge({ type }: { type: ReadingItem['type'] }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 text-[10px] font-medium uppercase tracking-widest text-gray-600 dark:text-white/60 group-hover:text-gray-900 dark:group-hover:text-white/85 transition-colors">
    ...
    <section className="px-4 sm:px-6 py-10">
      <div className="mx-auto max-w-2xl">
        ...
        <div className="flex items-center gap-4 mb-8">
          <h3 className="text-[15px] font-semibold tracking-tight text-gray-900 dark:text-white/90 whitespace-nowrap">
            Currently reading
          </h3>
          <div className="flex-1 h-px bg-gradient-to-r from-white/40 dark:from-white/10 to-transparent" />
          <Link
            href="/reading"
            className="text-[11px] text-gray-600 dark:text-white/60 hover:text-gray-900 dark:hover:text-white/90 transition-colors whitespace-nowrap"
          >
            Reading list →
          </Link>
        </div>
                    ...
                    <div className="flex-1 flex flex-col justify-between relative z-10">
                      <div className="mb-3">
                        ...
                        <div className="flex items-center justify-between gap-2">
                          <p className="text-xs text-gray-600 dark:text-white/70 line-clamp-1">{item.author}</p>
                      ...
                      <div className="flex items-center justify-between gap-2">
                        ...
                        {item.hasContent && (
                          <span className="text-[11px] text-gray-600 dark:text-white/60 group-hover:text-gray-900 dark:group-hover:text-white/85 transition-colors flex-shrink-0">
                            Read notes →
                          </span>
                        )}
                      </div>
